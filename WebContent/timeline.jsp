					...
					<div class="account-name">
						...
						<c:if test="${ not empty loginUser && loginUser.id != message.userId }">
							<form action="follow" method="post">
								<input type="hidden" name="followingId" value="${message.userId}">
								<c:choose>
									<c:when test="${message.following}">
										<input type="submit" class="following-btn" value="フォロー中">
									</c:when>
									<c:otherwise>
										<input type="submit" class="follow-btn" value="フォロー">
									</c:otherwise>
								</c:choose>
							</form>
						</c:if>
